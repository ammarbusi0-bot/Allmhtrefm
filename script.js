...
const ALL_QUESTIONS_DATA = [
    ...
    {q: "ماذا يُقصد بـ 'المفلس' في الفقه؟", a: ["من زادت ديونه على ماله", "من لا يملك شيئاً على الإطلاق", "من يرفض سداد الديون", "من يبيع سلعته بأقل من سعرها"], correct: 0, info: "المفلس هو من حجر عليه القاضي لزيادة ديونه على أمواله."},
    ...
    {q: "ما الحكم إذا نوى المأموم متابعة الإمام ثم سبقه إلى الركوع؟", a: ["تبطل صلاته إذا تعمد", "يجب عليه إعادة الصلاة", "تصح صلاته مع الكراهة", "يجب عليه سجود السهو"], correct: 0, info: "مساواة الإمام أو سبقه في الأركان (غير تكبيرة الإحرام والسلام) يُكره، وقد تبطل صلاته إذا سبقه بركنين عند بعض الفقهاء."},
...
const profileBtn = document.getElementById('profile-btn'); // زر عرض الملف الشخصي
...
function showWelcomeScreen() {
    ...
    welcomeScreen.classList.add('active'); // تفعيل كلاس التأثير
    welcomeUsername.textContent = username;
    
    // عرض رسالة الترحيب مع أعلى نتيجة
    ...
    profileBtn.classList.remove('hidden'); // إظهار زر الإحصائيات
}
...
function initializeQuestions(type) {
    ...
    if (sourceData.length < QUIZ_LENGTH) {
        quizContainer.innerHTML = `<div class="card text-red-500 text-xl mt-12 active"><p>❌ لا توجد أسئلة كافية لهذا النوع من الاختبار.</p>
            <button id="return-to-welcome-error" class="main-btn mt-4">العودة إلى القائمة</button></div>`;
        document.getElementById('return-to-welcome-error').addEventListener('click', showWelcomeScreen);
        loadingMessage.classList.add('hidden');
        return;
    }

    // خلط الأسئلة (استخدام طريقة مبسطة للعشوائية لضمان عملها)
    ...
    const shuffledQuestions = simpleShuffle(sourceData);
    
    // اختيار 10 أسئلة وتهيئة خلط الإجابات
    selectedQuestions = shuffledQuestions.slice(0, QUIZ_LENGTH).map((q, index) => {
        ...
        const shuffledAnswers = simpleShuffle(answers);
...
function loadQuestion() {
    // إزالة كلاس active من البطاقة السابقة
    ...
    // تصميم أزرار الإجابة حسب نوع السؤال (تم تحديث الألوان لتناسب CSS الجديد)
    let btnBaseClass = 'answer-btn';
    ...
    // استخدام ألوان Tailwind مدمجة
    if (currentQuizType === 'tf') {
        // ألوان لأزرار صحيح وخطأ
        btnBaseClass += ' bg-red-600 hover:bg-red-500';
    } else {
        // ألوان لأزرار الاختيار المتعدد
        btnBaseClass += ' bg-emerald-700 hover:bg-emerald-600';
    }

    // تصميم البطاقة للسؤال التالي
    quizContainer.innerHTML = `
        <div class="card question-card mt-6" id="current-question-card">
            ...
            <div id="answers-container" class="${answersGridClass}">
                ${q.a.map((answer, index) => `<button class="${btnBaseClass}" data-index="${index}">${answer}</button>`).join('')}
            </div>
            <div id="info-box" class="hidden text-sm mt-4 text-right border-r-2 border-[#66fcf1] pr-3 p-2 bg-[#1f2833]/50 rounded-lg"></div>
            <div class="timer-bar mt-6"><div id="timer-fill" class="timer-fill" style="transition:width ${TIME_PER_QUESTION}s linear;"></div></div>
        </div>
    `;
    
    // تفعيل كلاس التأثير للبطاقة الجديدة
    setTimeout(() => {
        document.getElementById('current-question-card').classList.add('active');
    }, 50);
...
function startQuestionTimer() {
    ...
    questionTimer = setInterval(() => {
        ...
        if (questionCountdown <= 5) {
             timerNumber.classList.add('text-red-500'); // تغيير لون المؤقت عند اقتراب الانتهاء
        } else {
...
function handleTimeUp() {
    if (!answered) {
        ...
        // تعطيل جميع الأزرار ومنع التفاعل
...
function checkAnswer(event) {
    ...
    answered = true;
    clearInterval(questionTimer);
    timerNumber.classList.remove('text-red-500'); // إزالة اللون الأحمر
...
function showFinalResult() {
    ...
    else if (percentage >= 50) message = `نتيجة جيدة. ${score} من ${QUIZ_LENGTH}. يمكنك تحقيق نتيجة أفضل. 💡`;
    else message = `نتيجتك هي: ${score} من ${QUIZ_LENGTH}. تحتاج إلى مراجعة بسيطة. 📚`;
    
    // إنشاء واجهة النتيجة النهائية
    quizContainer.innerHTML = `
        <div class="card question-card mt-6 active p-8">
            ...
            <p class="text-lg mb-2 text-yellow-500">نتيجة هذه الجولة: ${score} / ${QUIZ_LENGTH} (${percentage.toFixed(2)}%)</p>
            <div class="w-full h-2 bg-gray-600 rounded-full mb-6 mx-auto max-w-sm">
...
function showProfileScreen() {
    ...
    quizContainer.innerHTML = `
        <div class="card question-card mt-6 active p-8 text-right">
            ...
            <button id="return-from-profile" class="main-btn mt-8 py-3 px-6 text-xl bg-gray-600 hover:bg-gray-500 text-white">
...
// *** 8. وظيفة تشغيل الموسيقى/إيقافها ***
function toggleMusic() {
    const musicIcon = document.getElementById('music-icon'),
        musicStatus = document.getElementById('music-status');
    if (backgroundMusic.paused) {
        // يجب أن نستخدم play() مع catch() لأن Autoplay قد يفشل
...
startMCQBtn.addEventListener('click', (e) => {
    initializeQuestions(e.target.dataset.type);
});
...
// معالج زر عرض الملف الشخصي (جديد)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // إظهار شاشة تسجيل الدخول فقط إذا لم يكن هناك اسم محفوظ
    if (!savedUsername) {
    ...
    backgroundMusic.play().catch(() => {
        // هذا طبيعي في المتصفحات الحديثة، سيتم تشغيلها عند النقر
    });
});
